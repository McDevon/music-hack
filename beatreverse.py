...
def convert(inputFile, outputFile, options):
    ...
    for i in range(0, len(timelist)-1):
        ...
        combined += track[start:end].reverse() if i % sequenceLength in indices else track[start:end]
...
def main():
    ...
    parser.add_option('-i', '--index', default="0", help='beat indices to reverse, default = 0')
    ...
    fileName = args[0]
    outputFile = ''.join(fileName.split('.')[:-1]) + '_beatreverse.mp3'
