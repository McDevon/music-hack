...
def convert(inputFile, outputFile):
    ...
    for i in range(0, len(timelist)-1):
        ...
        firstHalf = audiosegment_to_ndarray(split[:segmentLen/2])[0]
        secondHalf = audiosegment_to_ndarray(split[segmentLen/2:])[0]
...
def main():
    ...
    fileName = args[0]
    outputFile = ''.join(fileName.split('.')[:-1]) + '_swing.mp3'
